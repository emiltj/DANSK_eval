...
size_of_domains = read.csv("/Users/emiltrencknerjessen/Desktop/priv/DANSK_eval/output/Thesis/DANSK/test.csv") %>% filter(. ,X == "DOCS") %>% select(. ,c(conversation = Conversation, dannet = Dannet, legal = Legal, news = News, social_media = Social.Media, web = Web, wiki_and_books = Wiki.and.Books))
...
df_wide = df_wide %>%
  mutate(score = case_when(
    ...
    colname == "web" ~ as.character(size_of_domains[1,"web"]),
...
size_of_tags = read.csv("/Users/emiltrencknerjessen/Desktop/priv/DANSK_eval/output/Thesis/DANSK/test.csv") %>% select(., c(ent = X, across_domains = All.domains.combined)) %>% filter(!ent %in% c('ENTS', 'DOCS'))
